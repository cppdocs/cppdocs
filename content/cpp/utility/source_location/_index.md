...
Before `std::source_location`, functions that needed call-site information for logging, testing, or diagnostics typically relied on [predefined macros](/cpp/preprocessor/replace/#Predefined_macros) such as `__LINE__` and `__FILE__`, so that those macros would expand in the context of the caller. `std::source_location` provides a library type for the same purpose.
