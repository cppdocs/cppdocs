...
`std::shared_ptr` may be used with an [incomplete type](/cpp/language/incomplete_type/). However, constructing from a raw pointer and resetting from a raw pointer require the pointed-to type to be complete at the call site.
...
## Helper classes
- [`std::atomic<std::shared_ptr>`](/cpp/memory/shared_ptr/atomic2/): atomic wrapper for `shared_ptr` objects (since C++20)
- [`std::hash<std::shared_ptr>`](/cpp/memory/shared_ptr/hash/): hash support for `shared_ptr`
- [`enable_shared_from_this`](/cpp/memory/enable_shared_from_this/): safe creation of additional shared owners from `this`

## Deduction guides
- [`shared_ptr` deduction guides](/cpp/memory/shared_ptr/deduction_guides/): class template argument deduction support (since C++17)
